...
<form name="myFrm" action="removeForm.do">
	<input type="hidden" value="${board.boardNo }" name="bno"> <input
		type="hidden" value="${page }" name="page"> <input
	...
	<table class="table">
		...
		<tr align="right">
			<td colspan="3"><c:choose>
					...
					<c:otherwise>
						<button type="button" disabled class="btn btn-warning">수정
							페이지</button>
						<button type="submit" disabled class="btn btn-danger">삭제
							페이지</button>
					</c:otherwise>
				</c:choose> <buntton type="button" class="btn btn-primary"
					onclick="location.href='boardList.do?page=${page}&searchCondition=${searchCondition}&keyword=${keyword}'">목록 이동</buntton></td>
		</tr>
